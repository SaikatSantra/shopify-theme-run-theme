...
const createTemplate = () => {
  if (!args.name) {
    ...
    console.log(
      `use ${chalk.blue("npm run create:template -- --name=your-template-name")} replacing the name value`,
    );
    return;
  }
  ...
  const templateFile = `./theme/templates/${args.name}.json`;
  let styleInclude = "";
  let stylesFile = "";
  ...
  if (args.folder) {
    ...
    fs.mkdir(
      `./src/styles/templates/${args.folder}`,
      { recursive: true },
      (err) => {
        if (err) throw err;
      },
