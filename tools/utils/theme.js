...
const getThemes = (store, username, pass) => {
  return axios.get(`https://${username}:${pass}@${store}/admin/api/${API_VERSION}/themes.json`)
    .then(response => {
      if (response.data) {
...
const deleteTheme = (themeId, store, username, pass) => {
  return axios.delete(`https://${username}:${pass}@${store}/admin/api/${API_VERSION}/themes/${themeId}.json`)
...
const deleteThemeByName = (themeName, store, username, pass) => {
  return getThemes(store, username, pass)
    .then((themes) => {
      const theme = themes.find(theme => theme.name === themeName);
      if (theme) {
        return deleteTheme(theme.id, store, username, pass);
      }
      console.log(`Trying to delete theme ${themeName} but no themes with such name were found`);
    });
}
