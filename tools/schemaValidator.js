...
const lintSections = (specificPath) => {
  const sectionFolder = path.resolve("./theme/sections");
  const configFolder = path.resolve("./theme/config");
  ...
  const validateFile = (folder, file) => {
    ...
    if (
      rawContents.indexOf("{% schema %}") > -1 ||
      file.indexOf(".json") > -1
    ) {
      const schema = rawContents
        .split("{% schema %}")
        .pop()
        .split("{% endschema %}")[0];
      console.log("Testing: ", colors.blue(file));
      if (rawContents.indexOf("/*") > -1) {
        const schemaanother = schema.split("/*").pop().split("*/")[1];
        jsonlint.parse(schemaanother);
      } else {
