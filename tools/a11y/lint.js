...
} = require("./tests/click-toggles-on-untabbables");
...
const cleanLiquid = (liquid) => {
  ...
  const stringsReplaced = replaceBetween(
    tagsRemoved,
    ["{{", "}}"],
    "somestringvalue",
  );
...
const lintFile = (fileName) => {
  ...
  const createErrorLogger = ({ message, line }) => {
    ...
    if (lines[line - 2] && lines[line - 2].includes("@a11y-lint-ignore"))
      return [];
    return () => {
  ...
  return tests.flatMap((test) =>
    testWrapper(dom, test).flatMap(createErrorLogger),
  );
};
...
glob("**/*.liquid", {}, (_er, files) => {
  ...
  if (errorLoggers.length > 0) {
    ...
    console.log(
      `\x1b[32m PASSED. No errors found! Nice work. Be sure to audit on rendered theme code too.`,
    );
  }
});
