...
const createSection = () => {
  if (!args.name) {
    ...
    console.log(`use ${chalk.blue('npm run create:section -- --name=your-section-name')} replacing the name value`);
    return;
  }
...
{% endschema %}`;
  ...
  if (args.folder) {
    ...
    fs.mkdir(`./src/styles/sections/${args.folder}`, { recursive: true }, err => {
      if (err) throw err;
    });
  } else {
