...
const MultiOptionFilter: React.FC<IMultiOptionFilter> = ({filterIndex, filter}): JSX.Element => {
  ...
  return (
    <>
      <div className={`filter__options filter__options--multi filter__options--${filter.displayType.toLowerCase()}`}>
        <div className={`filter__items filter__items--${filter.displayType.toLowerCase()}`}>
          {
            (filter.options as ISearchFilterOption[]).map((option, index) => {
              ...
              return (
                <label className={`filter__option filter__option--multi  ${option.selected ? 'filter__option--selected' : ''} ${disabled ? 'filter__option--disabled' : ''} filter__option--${filter.displayType.toLowerCase()}`} key={index}>
                  <input onChange={() => {
                    inputEventHandlers.handleToggleMultiOption(filterIndex, index)
