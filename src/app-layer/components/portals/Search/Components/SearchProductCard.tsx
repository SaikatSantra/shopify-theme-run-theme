...
import { ISearchAndFilterProduct, ISearchAndFilterProductImage } from '../../../../context/search/types'
...
const SearchProductCard: React.FC<ISearchResultsProductCardComponent> = ({ product, dataSet, simple }): JSX.Element => {
  ...
  const productImageArray = product.media ? product.media.filter(media => media.media_type === 'image') : product.images;
  const currCollHandle = dataSet?.currCollectionHandle ? dataSet.currCollectionHandle : null
  ...
  const productHandle = product.handle ? product.handle : product.url.split('products/')[1]

  const productUrl = `${getRoute()}products/${productHandle}`

  const otherImgs: ISearchAndFilterProductImage[] = productImageArray.filter(img => img.src !== mainImg.src);
  ...
  useEffect(() => {
    const storeAllProductsData = async (): Promise<void> => {
      ...
      const data = await getTemplateJSON<any>('products', productHandle, 'json-data');
      if (data) {
  ...
  return (
    productMetafields ? (
      <div className={`product-card product-card--react${simple ? ' product-card--simple' : ''}`}>
        <div className='product-card__image-container'>
          <a href={dataSet.customer==='logged-in'?productUrl:'/account/login'}>
            <div className="product-card__image">
              {
                productImageArray && productImageArray.length > 0 && (
                  <Image alt={mainImg.alt ? mainImg.alt : product.name} width={300} height={300} src={mainImg.src} className='product-card__img' />
                )
              }
              {
                otherImgs && otherImgs.length > 0 && (
                  <Image alt={otherImgs[0].alt ? otherImgs[0].alt : product.name} width={300} height={300} src={otherImgs[0].src} className='product-card__img img-hover' />
                )
              }
              { productMetafields && productMetafields.stickers && Object.values(productMetafields.stickers).length ? (
        ...
        <div className="product-card__info">
          <h3 className="product-card__title"><a href={dataSet.customer==='logged-in'?productUrl:'/account/login'}>{product.name}</a></h3>

          <div className="product-card__price">
            <Price
              finalPrice={product.priceObj ? product.priceObj.finalPrice : product.price}
              originalPrice={product.priceObj ? product.priceObj.originalPrice : product.compare_at_price ? product.compare_at_price : product.price_max}
              priceMin={product.priceObj ? product.priceObj.priceMin : product.price_min}
              priceMax={product.priceObj ? product.priceObj.priceMax : product.price_max}
              priceVaries={product.priceObj ? (product.priceObj.priceMin !== product.priceObj.priceMax) : (product.price_min !== product.price_max)}
            />
          </div>

          { productMetafields && productMetafields.swatches && Object.values(productMetafields.swatches).length ? (
