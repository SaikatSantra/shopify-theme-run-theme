...
const SearchFilterQuickRemoves: React.FC = (): JSX.Element => {
  ...
  const activeFiltersLength =
    getCurrentActiveFilters(searchFilters ?? [])?.length ?? 0;
      ...
      if (filter.type === "RANGE") {
        ...
        return (
          <button
            ...
            className="filter__quick-remove filter__quick-remove--range"
            key={filterIndex}
          >
            {filter.label}
            {": "}
        ...
        return filter.options.map((option, optionIndex) => {
          ...
          return (
            <button
              ...
              className="filter__quick-remove"
            >
              {label}
              <span className="filter__quick-remove-icon">✕</span>
            </button>
        ...
        <>
          <div className="filter__quick-removes-filters hide-medium-up">
            <>
              ...
              {activeFiltersLength > NUMBER_TO_SHOW && (
                <span>+{activeFiltersLength - NUMBER_TO_SHOW}</span>
              )}
            </>
            {/* {displayAllMob && <>
