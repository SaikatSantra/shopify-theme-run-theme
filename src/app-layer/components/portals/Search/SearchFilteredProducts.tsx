...
const PaginationNavLink: React.FC<PaginationLinkProps> = ({
  active,
  index,
  children,
}): JSX.Element => {
  ...
  const childProps = {
    onClick: () =>
      active
        ? null
        : inputEventHandlers.handleUpdatePageIndex(index, true, false),
  };
...
const ArrowPrev: React.FunctionComponent = (): JSX.Element => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 7 12">
    <path
      fill="#777"
      d="M0 6.004a.773.773 0 0 1 .167-.458L4.924.206A.6.6 0 0 1 5.354 0a.575.575 0 0 1 .438.186.66.66 0 0 1 .021.915L1.47 6.004l4.363 4.903a.672.672 0 0 1 .168.462.67.67 0 0 1-.189.454.608.608 0 0 1-.438.177.59.59 0 0 1-.43-.2L.188 6.464A.684.684 0 0 1 0 6.005L0 6.004Z"
    />
  </svg>
);
...
const ArrowNext: React.FunctionComponent = (): JSX.Element => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 7 12">
    <path
      fill="#777"
      d="M6.001 6.004a.773.773 0 0 0-.166-.458L1.078.206A.6.6 0 0 0 .648 0a.575.575 0 0 0-.439.186.66.66 0 0 0-.021.915l4.343 4.904-4.363 4.903A.672.672 0 0 0 0 11.37a.67.67 0 0 0 .19.454.608.608 0 0 0 .437.177.59.59 0 0 0 .43-.2l4.757-5.338A.684.684 0 0 0 6 6.005l.001-.001Z"
    />
  </svg>
);
...
const SearchFilteredProducts: React.FC<Props> = ({ dataSet }) => {
  ...
  const pageCount = productSearchResult
    ? Math.ceil(productSearchResult.total / config.rProductsPerPage)
    : 0;
  ...
  if (
    ...
    if (pageToRender === requestedPage) {
      ...
      productsToRender = productSearchResultsState
        .slice(requestedPage)
        .map((results) => results.products);
    } else if (pageToRender < requestedPage) {
      productsToRender = productSearchResultsState[pageToRender]?.products
        ? [productSearchResultsState[pageToRender].products]
        : [];
    }
  } else {
    // needs to be an empty array if not found (undefined within an array makes the length 1 :facepalm: )
    productsToRender = productSearchResult?.products
      ? [productSearchResult.products]
      : [];
  }

  if (productsToRender.length < 1 && pageToRender < requestedPage) {
    const liquidDummies = document.querySelector(
  ...
  return (
    <>
      {productsToRender.length > 0 ? (
        productsToRender[0].length === 0 ? (
          <p className="search__no-results">
            <Interweave content={noResultsSearchPage} noWrap={true} />
          </p>
        ) : (
          <div
