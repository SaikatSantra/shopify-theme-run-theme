...
const GiftWithPurchaseProduct: React.FunctionComponent<Props> = ({langStrings}: Props) => {
  const { giftWithPurchaseTitle } = langStrings;
  const [giftWithPurchaseProduct, setGiftWithPurchaseProduct] = React.useState<Product>(null);
  const [giftWithPurchaseVariant, setGiftWithPurchaseVariant] = React.useState<ProductVariant>(null);
  ...
  useEffect(() => {
    ...
    const upsellExistsInCart = handles.some(item => upsellHandles.includes(item))
    // If it is, return
    if (upsellExistsInCart) return;

    // See if we have a product that has an upsell
    const upsellParent = items.find((item : LineItem) => item.properties?._giftWithPurchaseHandle && !handles.includes(item.properties?._giftWithPurchaseHandle))

    const getUpsellProductData = async () => {
      const response = await axiosGetFullProductJSON(upsellParent.properties._giftWithPurchaseHandle)
      if (response) {
  ...
  return (
    <div className="minicart__gwp">
      ...
      <div className="minicart__gwp-item">
        {giftWithPurchaseProduct.image.src ? <Image alt={giftWithPurchaseProduct.title} width={80} height={80} src={giftWithPurchaseProduct.image.src} /> : ''}
        <div className="minicart__gwp-content">
          <div className="minicart__gwp-info">
            <h3 className="minicart__gwp-title label">{ giftWithPurchaseProduct.title }</h3>
            <div className="minicart__gwp-price label label--bold">
              ...
              <Price
                ...
                priceMax={giftWithPurchaseVariant.compare_at_price ? giftWithPurchaseVariant.compare_at_price : giftWithPurchaseVariant.price}
                originalPrice={giftWithPurchaseVariant.compare_at_price ? giftWithPurchaseVariant.compare_at_price : giftWithPurchaseVariant.price}
                finalPrice={0}
                priceVaries={false}
                withCurrency={false}
              />
