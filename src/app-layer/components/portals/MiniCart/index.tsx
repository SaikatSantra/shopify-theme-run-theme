...
const MiniCart: React.FunctionComponent<Props> = ({ dataSet }: Props) => {
  const { closeMinicart, cart, items, updating, minicartOpen, updateNote, updateCartAttributes } = useCart()
  const {
    title,
    summaryTitle,
    removeText,
  ...
  return (
    <>
      <button tabIndex={-1} onClick={closeMinicart} aria-label="Close" className="minicart__underlay"></button>
      <aside className="minicart">
        ...
        <header className="minicart__header">
          <div className="minicart__title-wrap">
            <h4 className="minicart__title heading-2">{ title } { updating ? (<LoadingIcon />) : '' }</h4>
        ...
        <div className="minicart__scroll-area">
          ...
          {shippingRows.length > 0 && (
            <div className={`delivery-info__container${isDeliveryInfoOpen ? ' delivery-info__container--open' : ''}`}>
              <button
              ...
              <div className='delivery-info__content'>
                {shippingRows.map((row, index) => {
                  return (
                    <p className='label label--sm' key={index}><span>{row.title}</span><span>{row.price}</span></p>
                  );
                })}
                {shippingLink &&
                  shippingLink.map((link, index) => {
                    return (
                      link.link && link.text ? (
                        <p key={index}><a className="link" href={link.link}>{link.text}</a></p>
                      ) : null
                    );
                  })}
              </div>
          ...
          <div className="minicart__scroll-area-inner">
            <div className={cartItems ? 'minicart__main' : 'minicart__main minicart__main--empty'}>
              {cartItems ?
                <>
                  ...
                  {showCartUpsell &&
                    <GiftWithPurchaseProduct langStrings={{ giftWithPurchaseTitle }} />
                  }
                  <GiftProduct
                    langStrings={{
                      giftProductTitle,
                ...
                <div className="minicart__empty">
                  <div className="minicart__empty-content">
                    ...
                    <a href={`${getRoute()}collections/all`} className="cta">{shopNow}</a>
                  </div>
                </div>
              }
            </div>
            {cartItems ?
              <CartUpsells dataSet={dataSet} />
              : null}
          </div>
        </div>
        {cartItems && dataSet.cartNotes === 'true' ?
          <div className={`minicart__accordion${isOrderNoteOpen ? ' minicart__accordion--open' : ''}`}>
            <button
              ...
              <div className="input-wrapper">
                <textarea
                  ...
                  onChange={e => {
                    ...
                    updateNote(e.target.value).then(() => setEnableCheckout(true))
                  }}></textarea>
              </div>
              <p className="small">{noteSupportText.replace('{{ max }}', dataSet.cartNotesLength)}</p>
            </div>
          </div>
          : null}
        {cartItems && dataSet.giftMessage === 'true' ?
          <div className={`minicart__accordion${isGiftMessageOpen ? ' minicart__accordion--open' : ''}`}>
            <button
              className='minicart__accordion-toggle'
              onClick={() => {
            ...
            <div className='minicart__accordion-content'>
              <div className="input-wrapper">
                <textarea
                  ...
                  onChange={e => {
                    setEnableCheckout(false)
                    updateCartAttributes({ GiftMessage : e.target.value}).then(() => setEnableCheckout(true))
                  }}></textarea>
              </div>
              <p className="small">{giftMessageSupportText.replace('{{ max }}', dataSet.giftMessageLength)}</p>
            </div>
          </div>
          : null}
        {cartItems ?
          <footer className="minicart__summary">
            <div className="minicart__summary-title heading-3">{ summaryTitle }</div>
            { cart.total_discount > 0 ?
              <>
                <div className="minicart__footer-line">
                  <span>{ orderTitle } ({cart.item_count} {itemsText})</span><span><Money amount={cart.original_total_price} /></span>
                </div>
                <div className="minicart__footer-line">
                  <span>{savingTitle}</span><span><Money amount={cart.total_discount} /></span>
                </div>
              </>
              : null}
            <div className="minicart__footer-line minicart__totals">
              <span>{subTotalTitle}</span><span><Money amount={ cart.total_price } /></span>
            </div>
            <div className="minicart__ctas">
              <a href={`${getRoute()}checkout`} className={`btn btn--secondary minicart__cta ${enableCheckout ? '' : 'btn--disabled'}`}>{ proccedCta }</a>
              <a href={`${getRoute()}cart`} className="btn btn--primary minicart__cta">{ viewCartCta }</a>
            </div>
            <div className="minicart__additional-text rte">
              { additionalMinicartHTML && <Interweave content={additionalMinicartHTML} noWrap={true} />}
            </div>
          </footer>
          : null}
      </aside>
