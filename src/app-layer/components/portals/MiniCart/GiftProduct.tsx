...
const GiftProduct: React.FunctionComponent<Props> = ({ langStrings }: Props)=> {
  const {
    giftProductTitle,
  ...
  const addSelectedVariant = (selectedVariantId: string) => {
    ...
    const variantData = giftProductData.variants && giftProductData.variants.find(variant => {
      return variant.id === parseInt(selectedVariantId)
    })
    setSelectedVariant(variantData)
  }

  const disableATC = (dropdownValue: string, selectedVariant: ProductVariant) => {
  ...
  useEffect(() => {
    const getGiftProductData = async () => {
      ...
      if (response) {setGiftProductData(response)}
    }
  ...
  return (
    giftProductData ? (
      <div className="gift-product">
        <Image alt={giftProductData.title} width={110} height={110} src={giftProductData.images[0]} />
        <div className="text-container">
          ...
          <Price
            ...
            priceVaries={false}
            withCurrency={true}></Price>
        </div>
        <button className="btn btn--primary" onClick={() => setShowModal(!showModal)}>{addText}</button>
            ...
            <div className="gift-product__modal">
              <div className="gift-product__inner">
                <div className="minicart-header">
                  ...
                  <button className="minicart-close-button" onClick={() => setShowModal(false) }>&times;</button>
                </div>
                <div className="gift-product__card">
                  <div className="gift-product__card-inner">
                    <Image alt={selectedVariant && selectedVariant.featured_image.src ? selectedVariant.name : giftProductData.title} width={110} height={110} src={selectedVariant && selectedVariant.featured_image.src ? selectedVariant.featured_image.src : giftProductData.images && giftProductData.images[0] ? giftProductData.images[0] : ''} />
                    <div className="gift-product__info">
                      <p>{selectedVariant && selectedVariant.name ? selectedVariant.name : giftProductData.title}</p>
                      <Price
                        originalPrice={selectedVariant ? selectedVariant.price : giftProductData.price}
                        finalPrice={selectedVariant ? selectedVariant.price : giftProductData.price}
                        priceVaries={false}
                        withCurrency={false}></Price>
                      <div className="select-wrapper">
                        <select className="select" onChange={e => setdropdownValue(e.target.value)} value={dropdownValue}>
                  ...
                  <input className="text-area" type="textarea" placeholder="Gift message..." onChange={e => setMessage(e.target.value)} />
                </div>
                <button
