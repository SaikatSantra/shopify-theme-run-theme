...
const CartRecommendedSelect = (props: CartRecommendedSelectProps): JSX.Element => {
  const { product } = props;
  ...
  const addSelectedVariant = (selectedVariantId: string) => {
    const variantData = product.variants && product.variants.find(variant => {
  ...
  if (product.variants.length > 1) {
    selectMarkUp = (
      <>
        <div className="select-wrapper">
          <label htmlFor={`product-options-${product.id}`} className="visually-hidden">
            {optString}
          </label>
          <select
          ...
          >
            ...
            {product.variants.map((variant) => {
              return (
                <option key={variant['id']} value={variant['id']} data-variant-available={variant['available']}>
                  {variant['available'] ? variant['title'] : window['theme'].strings.soldOut}
                </option>
              );
            })}
          </select>
    ...
    selectMarkUp = (
      <>
        ...
        >
          {product['available'] ? window['theme'].strings.addToCart : window['theme'].soldOut}
        </button>
      </>
    );
  }
