  ...
  return (
    <>
      {product.options.map(({ name, values, position }) => (
        <div className={`variant-option variant-option--${name.toLowerCase()}`} key={position}>
          ...
          <div className="variant-option__options">
            {values.map((value, i) => {
              ...
              const matchingVariants = getMatchingVariants(possibleOptions, product.variants)
              const matchingVariant = matchingVariants.length === 1 ? matchingVariants[0] : null
              const lowStock = Boolean(matchingVariant && stockMap && stockMap[matchingVariant.id.toString()] && stockMap[matchingVariant.id.toString()]['availability'] === 'low');
              const preOrder = Boolean(matchingVariant && stockMap && stockMap[matchingVariant.id.toString()] && stockMap[matchingVariant.id.toString()]['availability'] === 'pre-order');
