...
const QuickAdd: React.FunctionComponent<Props> = ({ dataSet }: Props) => {
  ...
  const { product, setHandle, setSelectedOptions,  selectedOptions, currentVariant, stockMap } = useQuickAdd()
  const { lowStockText, inStockText, oosText, fullDetailsCta, title, atcText, pleaseSelectText } = useLangStrings(dataSet.langStrings);
  ...
  const lowStock = Boolean(currentVariant && stockMap && stockMap[currentVariant.id.toString()] && stockMap[currentVariant.id.toString()]['availability'] === 'low');
  ...
  const optionsRemaining = product.options.filter((option, index) => option && !selectedOptions[index])

  const singleOptionRemaining = optionsRemaining.length === 1 ? optionsRemaining[0].name : ''

  const pleaseSelectTextReplaced = pleaseSelectText ? pleaseSelectText.replace('{{ option_name }}', singleOptionRemaining) : '';

  const actText = currentVariant ?
    currentVariant.available ? atcText: oosText : pleaseSelectTextReplaced
  ...
  return (
    <div className="quick-add__popup-wrap">
      <button tabIndex={-1} className="quick-add__underlay" aria-label='close' onClick={close}></button>
      <aside className="quick-add__popup">
        <header
          className="quick-add__header">
          <h3>{ title }</h3>
        ...
        <main className="quick-add__main">
          <div className="quick-add__image">
            <Image alt={product.title} width={182} height={182} src={product.featured_image} />
          </div>
          <div className="quick-add__info">
            ...
            */}
            <div className="variant-option__stock-display">
              {!currentVariant ? <span className="label variant-option__stock">{pleaseSelectTextReplaced}</span> : ''}
              {inStock && !lowStock ? <span className="label variant-option__stock variant-option__stock--in-stock">{inStockText}</span> : ''}
              {lowStock ? <span className="label variant-option__stock variant-option__stock--low-stock">{lowStockText}</span> : ''}
              {currentVariant && !inStock ? <span className="label variant-option__stock variant-option__stock--out-of-stock">{oosText}</span> : ''}
            ...
            <div className="quick-add__full-details-wrap">
              <a className='cta' href={`${getRoute()}products/${product.handle}`}>{ fullDetailsCta }</a>
            </div>
          </div>
        </main>
      </aside>
