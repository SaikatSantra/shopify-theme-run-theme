...
export const CartUpsells: React.FunctionComponent<Props> = ({ dataSet }: Props) => {
  ...
  useEffect(() => {
    ...
    axios.get(`/recommendations/products.json?product_id=${mostExpensiveItem.product_id}&limit=4&intent=related`)
      .then((res) => {
  ...
  if (dataSet.nostoUpsellId) {
    return (
      <div className="cart-upsells">
        ...
        <div className="nosto_element" data-nosto-elem id={dataSet.nostoUpsellId}></div>
      </div>
    )
  }
              ...
              <SwiperSlide key={product.id}>
                <div className="cart-upsell">
                  <div className="cart-upsell__image">
                    <Image alt={product.title} width={164} height={164} src={product.featured_image} />
                  </div>
                  <div className="cart-upsell__details">
                    <h4 className='heading-4'>{product.title}</h4>
                    <Price
                      finalPrice={product.price}
                      originalPrice={product.compare_at_price ? product.compare_at_price : product.price}
                      priceVaries={Boolean(product.price_varies)}
                      priceMin={product.min_price ? product.min_price : product.price} />
