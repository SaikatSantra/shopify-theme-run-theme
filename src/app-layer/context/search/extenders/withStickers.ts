...
const getStickers = (): IKeyedStickers => {
  const stickerScript = document.getElementById('stickerData') as HTMLScriptElement
  const stickerDataRaw = stickerScript.textContent
  return safeJSONParse(stickerDataRaw, {}) as IKeyedStickers
}

const getProductStickersFromTags = (stickers : IKeyedStickers, tags: string[]): ISticker[] => {
  ...
  return tags.flatMap(tag => {
    if (tag.includes('sticker-')) { // match for the sticker tag, See product-stickers.liquid
      if (stickers[tag]) {
...
const withStickers: IExtender = {
  ...
  extendGetProductsResult: (results) => {
    const updatedProducts = results.products.map(product => {
      const tags = [...product.tags, ...product.available ? [] : ['sticker-sold-out'] ]
