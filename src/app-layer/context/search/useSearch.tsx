...
import {
  ...
  IActiveCollection, IGetProductsResult,
...
interface ContextValue {
  ...
  inputEventHandlers: {
    ...
    handleUpdateRangeOption: (filterIndex: number, optionIndex: number, selectedMinimum: number, selectedMaximum: number) => void,
    handleToggleMultiOption: (filterIndex: number, optionIndex: number, force?: boolean) => void,
    handleToggleSingleOption: (filterIndex: number, optionIndex: number, value: boolean) => void,
    handleUpdatePageIndex: (pageIndex: number, triggerFiltering: boolean, silent: boolean) => void,
...
export const SearchProvider: React.FC<IFFetchContext> = ({ children, config, activeCollection }) => {
  ...
  // data
  const [quickSearchResult, setQuickSearchResult] = useState<IHandleQuickSearchResult>(null)
  const [quickSearchResultsTypos, setQuickSearchResultsTypos] = useState<IQuickSearchResultTypo[]>(null)
  const [quickSearchResultsProducts, setQuickSearchResultsProducts] = useState<ISearchAndFilterProduct[]>(null)
  const [quickSearchResultsCollections, setQuickSearchResultsCollections] = useState<IQuickSearchResultCollection[]>(null)
  const [quickSearchResultsSuggestions, setQuickSearchResultsSuggestions] = useState<IQuickSearchResultSuggestion[]>(null)
  const [quickSearchResultsContent, setQuickSearchResultsContent] = useState<IQuickSearchResultContent[]>(null)
  const [quickSearchResultsPages, setQuickSearchResultsPages] = useState<IQuickSearchResultPage[]>(null)
  const [quickSearchResultsArticles, setQuickSearchResultsArticles] = useState<IQuickSearchResultArticle[]>(null)
  ...
  const [productSearchResult, setProductSearchResult] = useState<IGetProductsResult>(null)
  const [productSearchResultsState, setProductResultsState] = useState<IGetProductsResult[]>([])

  const [cmsSearchResults, setCmsSearchResults] = useState<IGetCmsContentResult>(null)
  ...
  const [pendingProductCount, setPendingProductCount] = useState(null as null|number)


  const updateFilters = (newFilters: ISearchFilter[], activate?: boolean): void => {

    const updatedFilters = searchFilters.map(filter => {

      const newFilter = newFilters.find(({ identifier }) => identifier === filter.identifier)
      ...
      const newOptions  = newFilter ? newFilter.options as ISearchFilterOption[] : []

      return {
        ...filter,
        options: oldOptions.map((option) => {
          // Shopify needs to use the option value rather than identifier
          const newOption = newOptions.find(({ identifier }) => identifier && identifier === option.value ? option.value : option.identifier)
          return {
            ...option,
            ...activate ? {
  ...
  const refreshFiltersOnly = () => {
    SearchService.methods.getProductResults(
      searchFilters,
      activeCollection,
  ...
  const outputProductData = (productData: IGetProductsResult, checkPageIndex: number) => {
    ...
    if (config.rLoadMoreMode !== 'paginate' && checkPageIndex) {
      ...
      const allData = state.filter(def => def).reduce((all, thisData) => {
        return {
          ...
          //otherwise go and get thisData.raw.queryResults[0].records and spread that (this is used for Klevu v2)
            ...(Array.isArray(thisData.raw) ?
              thisData.raw : thisData.raw?.queryResults[0].records.length > 0 ?
  ...
  const executeFilteredSearch = useDebouncedCallback((forceIndex = null) => {
    ...
    if (checkPageIndex && productSearchResultsState[checkPageIndex]) {
      wipeLiquidProducts();
      outputProductData(productSearchResultsState[checkPageIndex], checkPageIndex)
  ...
  const updatePageIndex = (pageIndex: number, triggerFiltering: boolean, silent: boolean, historyMethod: 'pushState' | 'replaceState' = 'pushState') => {
    if (window['theme'].template === 'search' || window['theme'].template === 'collection' || window['ShopifyAnalytics'].meta.page.pageType === 'collection' || window['ShopifyAnalytics'].meta.page.pageType === 'search') {
  ...
  const eventHandlers = {
    filterUpdated: (params: {
      ...
      if (['SINGLE_OPTION', 'MULTI_OPTION'].includes(filter.type)) {
        ...
        // @ts-ignore
        const options: ISearchFilterOption[] = filter.options.map((option: ISearchFilterOption) => {
          // Shopify needs to use the option value rather than identifier
          return option.selected ? option.value ? option.value : option.identifier : false
        ...
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-ignore
        const option: ISearchFilterRangeOption = filter.options[params.optionIndex]
        const isActive = (option.selectedMinimum > option.minimum || option.selectedMaximum < option.maximum)
        if (isActive) {
          updateLocationQuery({
            key: `${FILTER_QUERY_PARAM_PREFIX}${filter.identifier}`,
  ...
  const inputEventHandlers = {
    ...
    handleSearchSuggestionSelected: (index) => {
      ...
      const suggestion = quickSearchResultsSuggestions[index].value.replace(/(<([^>]+)>)/gi, '')
      setIsSearching(true)
      setSearchTerm(suggestion)
      executeSearch.callback(suggestion)
    },

    handleUpdateRangeOption: (filterIndex: number, optionIndex: number, selectedMinimum: number, selectedMaximum: number) => {

      const updatedOption = [...searchFilters][filterIndex].options[optionIndex] as ISearchFilterRangeOption
      ...
      setFiltersToUpdate([...filtersToUpdate, {
    ...
    handleToggleMultiOption: (filterIndex: number, optionIndex: number, forceValue?: boolean) => {


      const updatedOption = [...searchFilters][filterIndex].options[optionIndex] as ISearchFilterOption

      updatedOption.selected = typeof forceValue === 'undefined' ? !updatedOption.selected : forceValue
    ...
    handleToggleSingleOption: (filterIndex: number, optionIndex: number, value: boolean) => {

      const updatedFilterOptions = [...searchFilters][filterIndex].options as ISearchFilterOption[];
    ...
    handleUpdatePageIndex: (pageIndex: number, triggerFiltering: boolean, silent: boolean) => {
    ...
    handleClearSpecificFilter: (filterIndex: number) => {
      ...
      if (filterToClear.type === 'RANGE') {
        ...
        const indices = []
        const ticked = (filterToClear.options as ISearchFilterOption[]).map(({selected}, i) => ({selected, i}))
        if (ticked.length) {
        ...
        indices.forEach(({ int }) => inputEventHandlers.handleToggleMultiOption(filterIndex, int))
      }
    ...
    handleClearAllFilters: (): void => {
      ...
      for (let i = 0; i < updatedFilters.length; i++) {
        ...
        for (let j = 0; j < filter.options.length; j++) {
          const option: ISearchFilterRangeOption | ISearchFilterOption = filter.options[j];
    ...
    handleToggleFilterOpen: (filterIndex: number, value?: boolean) => {
      const isDesktop = window.matchMedia('(min-width: 768px)').matches ? true : false;
      const updatedFilters = [...searchFilters].map((filter, index) => {
        if (index !== filterIndex) {
          //close others
          return {
            ...filter,
            open: isDesktop ? filter.open ? !filter.open : filter.open : filter.open
          }
        } else {
          return {
            ...filter,
  ...
  const getCurrentUrlWithTagSet = (tag: string, val: boolean) => {
    ...
    if (val && path[path.length - 1] !== tag) { // needs to add
      newPath = `/${path.join('/')}/${tag}`
    } else if (!val && path[path.length - 1] === tag) { // needs to remove
  ...
  useEffect(() => {
    ...
    const now = new Date();
    const localCollections = JSON.parse(window.localStorage.getItem('bb-collections'));
    if (!localCollections || (localCollections && localCollections['00-timer'] && now > localCollections['00-timer'])) {
      const storageTimeMinutes = 10;
      // used to populate list of collection urls and their ids for use in collecitonOptionFilter
      axios.get('/collections/all?view=json')
        .then((res) => {
          ...
          // add an additional timer element to the array so we can store for later checking and invalidate after a time
            window['theme']['collections'] = {...{'00-timer': new Date(now.getTime() + storageTimeMinutes*60000)}, ...res.data.collections}
            // This is not pretty but is the easiest fix for now - sorry
            axios.get('/collections/all?page=2&view=json')
              .then((res) => {
                if (res.data?.collections) {
                  window['theme']['collections'] = {...window['theme']['collections'], ...res.data.collections}
                  window.localStorage.setItem('bb-collections', JSON.stringify(window['theme']['collections']))
  ...
  useEffect(() => {
    ...
    if (!cmsSearchResults && params.get('q')) {
      SearchService.methods.getCmsContent(params.get('q'), config).then((response) => {
    ...
    if (!searchFilters && (window['theme'].template === 'search' || window['theme'].template === 'collection')) {

      SearchService.methods.getFilters(activeCollection, localSearchTerm, config).then((response) => {
        //klevu min & max ranges coming back as strings and breaking range slider
        const rangeFilters = response.filters.find(filter => filter.displayType === 'RANGE')
        if (rangeFilters?.options?.length > 0) {
          const rangeOption = rangeFilters.options[0] as ISearchFilterRangeOption
        ...
        if (selectedFilters.length > 0) {
          selectedFilters.forEach((selectedFilter: {
            ...
            const filterIndex = response.filters.findIndex((filter) => {
              return `${FILTER_QUERY_PARAM_PREFIX}${filter.identifier}` === selectedFilter.filterKey
            })
            ...
            // toggle the option
            if (['SINGLE_OPTION', 'MULTI_OPTION'].includes(response.filters[filterIndex].type)) {

              // find the option index
              const optionIndex = response.filters[filterIndex].options.findIndex((option) => {
                // Shopify needs to use the option value rather than identifier
                return option.value ? option.value : option.identifier === selectedFilter.optionKey
              })


              if (optionIndex >= 0) {
                const option = (response.filters[filterIndex] as ISearchFilterStandard).options[optionIndex];
                option.active = true
                option.selected = true
                executeSearch = true

              }
            } else if (['RANGE'].includes(response.filters[filterIndex].type)) {
              const option = (response.filters[filterIndex] as ISearchFilterRange).options[0];
              option.selectedMinimum = parseFloat(selectedFilter.optionKey.split(':')[0]);
              option.selectedMaximum = parseFloat(selectedFilter.optionKey.split(':')[1]);
    ...
    const selectedFilters = searchFilters ? getCurrentActiveFilters(searchFilters) : [];
    const updateUrlWithTag = Boolean(productSearchResult && activeCollection.handle && (selectedFilters.length > 0 || sortKey))

    const url = getCurrentUrlWithTagSet(config.searchFilterDummyTag, updateUrlWithTag)
