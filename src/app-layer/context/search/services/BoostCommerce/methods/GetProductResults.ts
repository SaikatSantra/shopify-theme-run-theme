  ...
  new Promise<IGetProductsResult>((resolve) => {
    ...
    try {
      ...
      searchFilters.forEach((filter) => {
        filter.options.forEach((option) => {
          if (["MULTI_OPTION", "SINGLE_OPTION"].includes(filter.type)) {
            ...
            params[filter.identifier].push(
              `${option.selectedMinimum}:${option.selectedMaximum}`,
            );
          } else {
      ...
      BoostCommerceApi(config.apiKey, config.apiEndpoint)
        .request(
          window["theme"].template === "search" ? "search" : "filter",
          params,
        )
        .then((res) => {
          ...
          response.products = res.data.products.map((product, index: number) =>
