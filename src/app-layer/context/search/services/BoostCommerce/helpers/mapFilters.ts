...
const mapFilters = (response: {
  ...
  return response.data.filter.options.flatMap((filter) => {
    ...
    const prefix: RegExp | null = filter.prefix ? new RegExp(filter.prefix) : null;
    ...
    switch (filter.displayType) {
      ...
      case 'list':
      case 'rating':
        f.type = filter.selectType === 'multiple' ? 'MULTI_OPTION' : 'SINGLE_OPTION'
        f.displayType = 'TEXT'
