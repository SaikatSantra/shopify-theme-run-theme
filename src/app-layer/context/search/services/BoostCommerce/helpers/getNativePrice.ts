...
const getNativePrice = (product): ISearchAndFilterNativeProductPrice => {
  ...
  const price_min = standardisePrice(
    product[`price_min_${activeCurrency}`] ?? product.price_min,
  );
  const price_max = standardisePrice(
    product[`price_min_${activeCurrency}`] ?? product.price_min,
  );
  ...
  const compare_at_price_min =
    product.compare_at_price_min || `compare_at_price_min_${activeCurrency}`
      ? standardisePrice(
          product[`compare_at_price_min_${activeCurrency}`] ??
            product.compare_at_price_min,
        )
      : 0;
  const compare_at_price_max =
    product.compare_at_price_max || `compare_at_price_max_${activeCurrency}`
      ? standardisePrice(
          product[`compare_at_price_max_${activeCurrency}`] ??
            product.compare_at_price_max,
        )
      : 0;
  const compare_at_price_varies =
    compare_at_price_min === compare_at_price_max ? false : true;

  // Now we have the values from above we can compare them and get the original pricing
  const price =
    price_max > compare_at_price_max ? price_max : compare_at_price_max;
