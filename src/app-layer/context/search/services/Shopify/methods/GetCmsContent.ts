...
import { ISearchTemplateResponse, ITemplateArticleResponse, ITemplatePageResponse } from '../types';
...
const GetCmsContent = async (term: string): Promise<IGetCmsContentResult> => {
  ...
  try {
    const res = await getTemplateJSON<ISearchTemplateResponse>('search', '', 'sf-data', `&q=${term}&type=article,page`);

    // This is needed because the different type of results are returned together.
    const pages: ITemplatePageResponse[] = res.searchResults.filter(result => result.resultType === 'page');
    const articles: ITemplateArticleResponse[] = res.searchResults.filter(result => result.resultType === 'article');
