...
const GetProductResults = (
  ...
  new Promise<IGetProductsResult>((resolve) => {
    ...
    const filters = searchFilters
      .map((filter) => {
        ...
        if (["MULTI_OPTION", "SINGLE_OPTION"].includes(filter.type)) {
          transformedFilter.values = (filter.options as ISearchFilterOption[])
            .map((option) => {
          ...
          transformedFilter.values = [
            option.selectedMinimum,
            option.selectedMaximum,
          ];
        } else {
          return false;
        }
    ...
    try {
      KlevuApi(config.apiKey, config.apiEndpoint)
              ...
              settings: {
                ...
                offset:
                  ...
                  config.rLoadMoreMode === "infinite"
                    ? (query.pageIndex - 1) * config.rProductsPerPage
                    : 0,
                query: {
                  term: getSearchTerm(),
                },
        ...
        .then((res) => {
          ...
          // Map products
          if (
            typeof res.data.queryResults !== "undefined" &&
            ...
            response.products = res.data.queryResults[0].records.map(
              (record, index) => {
                return <ISearchAndFilterProduct>{
                  id: record.id,
                  index:
                    (query.pageIndex - 1) * config.rProductsPerPage + index,
                  ...
                  price_min:
                    record.startPrice === ""
                      ? standardisePrice(record.price)
                      : standardisePrice(record.startPrice),
                  price_max:
                    record.toPrice === ""
                      ? standardisePrice(record.price)
                      : standardisePrice(record.toPrice),
                  price_varies:
                    record.startPrice !== "" &&
                    record.toPrice !== "" &&
                    record.startPrice !== record.toPrice
                      ? true
                      : false,
