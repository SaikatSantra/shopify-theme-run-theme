...
const HandleSearch = (
  ...
  new Promise<IHandleQuickSearchResult>((resolve) => {
    const requestIds = {
    ...
    try {
      KlevuApi(config.apiKey, config.apiEndpoint)
        ...
        .then((res) => {
          ...
          // Map suggestions
          if (
            typeof res.data.suggestionResults !== "undefined" &&
            res.data.suggestionResults[0].suggestions.length > 0
          ) {
            response.suggestions =
              res.data.suggestionResults[0].suggestions.map((suggestion) => {
                return <IQuickSearchResultSuggestion>{
                  value: suggestion.suggest,
                  query: suggestion.suggest
                    .replace(/(<([^>]+)>)/gi, "")
                    .replace(/ /g, "+"),
                };
              });
                    ...
                    return <ISearchAndFilterProduct>{
                      ...
                      images: [
                        <ISearchAndFilterProductImage>{
                          src: record.imageUrl
                            .replace("_medium", "")
                            .split("?")[0], //gives us full size image
                        },
                      ],
