...
import { getCartSuccess, addToCartFail, setCartActive, setMinicart, setCartUpdating } from '../store/actions/cart';
...
export interface ICartHook {
  ...
  updateItem: (key: string, opts: ICartUpdateByKey) => Promise<{ newCart: Cart; possibleShortStock: boolean } | void>;
  removeItem: (key: string) => Promise<Cart | void>;
  addToCart: (items: ICartItem[], notify?: boolean, callback?: (cart: Cart) => void) => Promise<Cart | void>;
...
export const useCart = (): ICartHook => {
  ...
  // selectors
  const active: boolean = useSelector((state: RootState) => state.cart.cartActive)
  const minicartOpen: boolean = useSelector((state: RootState) => state.cart.minicartOpen)
  const cart: Cart = useSelector((state: RootState) => state.cart.current)
  const updating: boolean = useSelector((state: RootState) => state.cart.updating)
  ...
  const toggleMinicart = () => {
    const open: Cart = useSelector((state: RootState) => state.cart.minicartOpen)
  ...
  const getCartState = async (): Promise<Cart|void>   => {
    try {
      ...
      const cartState = assignRemainingAvailablities(shopifyCartState, variantsAtLimit)
      if (cartState) {
   ...
   */
  const addToCart = async (items: ICartItem[], notify = true, callback?: (cart: Cart) => void) : Promise<Cart|void> => {
    ...
    for (const item of items) {
      ...
      try {
        const itemAdded = await themeCart.addItem(id, { quantity, properties, sellingPlan })
    ...
    if (itemsAddedArray.length > 0) {
      if (notify) {
        window['blubolt'].minicart ? openMinicart() : dispatch(dispatchToast('cart', itemsAddedArray, quantityArray));
      }
      if (callback) {
        callback(newCart);
      }
  ...
  const updateCartByKey = async (key: string, opts: ICartUpdateByKey) => {
    ...
    if (!key || !opts) { return console.error('A key or options are required to update the cart') }
    ...
    try {
      ...
      if (opts.quantity) {
        ...
        if (updatedItem.quantity < opts.quantity) {
          ...
          dispatch(dispatchToast('error', { customMessage: `Cannot add more than ${updatedItem.quantity} of ${updatedItem.title}` }));
        }
      }
      dispatch(setCartUpdating(false))
  ...
  const removeByKey = async (key) => {
    dispatch(setCartUpdating(true))
    if (!key) { return console.error('A key is required to remove item from the cart') }
    try {
  ...
  const setDOMHelpers = () => {
    const themeHasMinicart = Boolean(document.querySelector(`[${CONSTANTS.PORTALS.MINICART}]`))
    window['blubolt'].cart = {
      currentCart: cart,
      getCartState,
      addToCart,
