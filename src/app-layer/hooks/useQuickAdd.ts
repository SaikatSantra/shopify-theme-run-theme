...
export const useQuickAdd = (): QuickAddHookResponse => {
  ...
  const [selectedOptions, setSelectedOptions] = useState([null, null, null] as OptionList)
  const [currentVariant, setCurrentVariant] = useState(null as null | ProductVariant)
  ...
  useEffect(() => {
    if (loading) {
      const button = document.querySelector(`[data-quick-add-btn="${loading}"]`)
  ...
  useEffect(() => {
    ...
    const matchingVariants = getMatchingVariants(selectedOptions, product.variants);
    const selectedLength = selectedOptions.reduce((acca, option) => option === null ? acca : acca + 1, 0)
    if (selectedLength === product.options.length && matchingVariants.length === 1) {
  ...
  useEffect(() => {
    ...
    if (handle) {
      ...
      axios.get(`${route}products/${handle}.js${currencyQuery}`).then(({ data }) => {
