...
export const productInit = async (options : typeof DEFAULTS, productUpdatedCallBack = (variant: ProductVariant) => variant): Promise<void> => {

  const opts  = { ...DEFAULTS, options }

  const productForms = document.querySelectorAll(opts.PRODUCT_FORM_SELECTOR) as NodeListOf<HTMLFormElement>
  ...
  productForms.forEach(async (productForm) => {
    //get the data
    const productData: Product | void = await axiosGetFullProductJSON(productForm.dataset.productForm)
    ...
    const variantIdFormField = productForm.querySelector(opts.PRODUCT_VARIANT_ID_FORM_FIELD) as HTMLInputElement
    const sellingPlanIdEl = productForm.querySelector(opts.PRODUCT_SELLING_PLAN_ID) as HTMLInputElement;
    const atcButton = productForm.querySelector(opts.PRODUCT_FORM_SUBMIT_BUTTON_SELECTOR) as HTMLButtonElement;
    const priceEls = productForm.querySelectorAll(opts.PRODUCT_PRICE_SELECTOR)
    const compareAtpriceEls = productForm.querySelectorAll(opts.PRODUCT_COMPARE_AT_PRICE_SELECTOR)
    const mainImage = productForm.querySelector(opts.PRODUCT_MAIN_IMAGE_SELECTOR) as HTMLImageElement
    const optionFields = productForm.querySelectorAll(opts.PRODUCT_OPTION_FIELD_SELECTOR) as NodeListOf<HTMLSelectElement|HTMLInputElement>
    const imageThumbs = productForm.querySelectorAll(opts.PRODUCT_IMAGE_THUMBS_SELECTOR)
    const lowStockLabel = productForm.querySelector('[data-variant-stock-display-low-stock]');
    const dummyAtcButton = productForm.querySelector(opts.PRODUCT_FORM_DUMMY_SUBMIT_BUTTON_SELECTOR);
    const dummySizeSelect = productForm.querySelector(opts.PRODUCT_DUMMY_OPTION_FIELD_SELECTOR) as HTMLSelectElement;
    const lowStockLimit = parseInt(productForm.dataset.lowStockLimit);

    const stockMap: boolean | any[] | Record<string, unknown> = (() => {
      try { return safeJSONParse(productForm.getAttribute(opts.PRODUCT_STOCK_MAP)) }
      catch (e) { console.info('No stockmap on product form'); return {} }
    })();

    const updateProductPrice = updatePrice(priceEls, compareAtpriceEls, window['theme'].moneyFormat)
    const updateProductVariantFormField = updateVariantFormField(variantIdFormField, productData.handle);
    ...
    const switchProductStockDisplay = switchStockDisplay({
      ...
      atcButton, oosAttr: opts.PRODUCT_OOS_TEXT_ATTR,
    ...
    const getAvailabilityFlag = (variant?: ProductVariant) => variant && stockMap[variant.id.toString()] ? stockMap[variant.id.toString()]['availability'] : 'unavailable';
    const getVariantQty = (variant?: ProductVariant) => variant && stockMap[variant.id.toString()] ? stockMap[variant.id.toString()]['qty'] : '0';
    const getSellingPlanId = (variant?: ProductVariant) => variant && stockMap[variant.id.toString()] ? stockMap[variant.id.toString()]['selling_plan_id'] : null;
    ...
    const updateLabel = () => {
      const variantSelectedEl = productForm.querySelectorAll('[data-option-select]:checked') as NodeListOf<HTMLInputElement>;
      variantSelectedEl.forEach((el) => {
        ...
        variantSelectedParent.querySelector('[data-variant-selected]').innerHTML = matchingLabel.innerHTML;
      });
    }

    const updateDummySelect = () => {
      const selectedVariant = (productForm.querySelector('.variant-option__radio:checked') as HTMLInputElement).dataset.optionValue;
    ...
    const updateProductToMatchThumbnail = (position: number) => {
      const matchingOptions = getMatchingOptionsFromImageIndex(optionsByImageIndex, position)
      const currentlySelectedOptions = getCurrentSelectedOptionsFromFields(optionFields)
      const closest = getClosestOptionsToCurrentlySelected(currentlySelectedOptions, matchingOptions)
      if (!closest) { return }
    ...
    const onFieldsChange = (fields: NodeListOf<HTMLSelectElement|HTMLInputElement>) => () => {
      const currentOptions = getCurrentSelectedOptionsFromFields(fields)
      const variant = getVariantFromVariantsAndOptions(productData.variants, currentOptions)
      updateVariantData(variant)
      if (!variant) {
        return
      }
    ...
    const onProductThumbnailClick = (thumbnail: Element) => () => {
      const position = getThumbnailPosition(opts.PRODUCT_IMAGE_THUMBS_ATTR, thumbnail)
      setActiveThumbnail(imageThumbs, opts.PRODUCT_IMAGE_THUMBS_ATTR, opts.PRODUCT_THUMBNAIL_ACTIVE_CLASS, position)
    ...
    const onDummySelectChange = (e: Event) => {
      ...
      const variantOption = productForm.querySelector(`input[data-option-value='${selectedOption}']`) as HTMLInputElement;
      variantOption.click();
    }
    ...
    dummySizeSelect ? dummySizeSelect.addEventListener('change', e => (onDummySelectChange(e))) : '';

    dummyAtcButton ? dummyAtcButton.addEventListener('click', () => (onDummyAtcClick())) : '';

    optionFields.forEach(select => select.addEventListener('change', onFieldsChange(optionFields)));

    imageThumbs.forEach(thumbnail => thumbnail.addEventListener('click', onProductThumbnailClick(thumbnail)));

    window['blubolt'].selectZoomImage = (imagePosition: number) => {
      const thumbnail = productForm.querySelector(`[data-product-thumbnail-index="${imagePosition}"]`);
      if (!thumbnail) {
    ...
    ['load', 'resize'].forEach(function(e) {
      window.addEventListener(e, () => {
        setTimeout(() => {
          const imagesCarousel = productForm.querySelector('.product-media-images .swiper') as HTMLElement;
          if (imagesCarousel) {
            const imagesFirstSlide = imagesCarousel.querySelector('.swiper-slide') as HTMLElement;
            const thumbsCarousel = productForm.querySelector('.product-media-thumbs .swiper') as HTMLElement;
    ...
    const highlightStockOptions = () => {
      ...
      productData.variants.forEach((variant) => {
        ...
        if (!variant.available || (!variant.selling_plan_allocations?.length && variantQty <= 0)) {
          oosVariantList.push(variant.title);
        }
      });
      const lowStockVariants = Array.from(optionFields).filter(option => lowStockVariantsList.includes(option.dataset.optionLabel));
      const preOrderVariants = Array.from(optionFields).filter(option => preOrderVariantsList.includes(option.dataset.optionLabel));
      const oosVariants = Array.from(optionFields).filter(option => oosVariantList.includes(option.dataset.optionLabel));

      lowStockVariants.forEach(variant => variant.parentElement.classList.add('variant-option__option--low-stock'));
      preOrderVariants.forEach(variant => variant.parentElement.classList.add('variant-option__option--pre-order'));
      oosVariants.forEach(variant => variant.parentElement.classList.add('variant-option__option--oos'));
    }

    highlightStockOptions();

    const initGiftCardRecipient = (): void => {
      const giftCardToggle = productForm.querySelector('[data-gift-card-toggle]') as HTMLInputElement;
      const giftCardDetails = productForm.querySelector('[data-gift-card-details]') as HTMLElement;
      if (!giftCardToggle || ! giftCardDetails) return;
      const checkGiftCardRecipientStatus = () => {
        const giftCardEmail = productForm.querySelector('[data-gift-card-email]') as HTMLInputElement;
        if (giftCardToggle.checked) {
          ...
          productForm.querySelector('[data-gift-card-form]').querySelectorAll('.text-input').forEach(input => {
