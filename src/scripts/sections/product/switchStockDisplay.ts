const switchStockDisplay =
  (options: Record<string, any>) =>
  (availablilityFlag: string): void => {
    const purchasable =
      availablilityFlag === "available" ||
      availablilityFlag === "pre-order" ||
      availablilityFlag === "low";
    const {
      stockWrapper,
      stockWrapperAttr,
      atcButton,
    ...
    const inStockLabel = document.querySelector(
      "[data-variant-stock-display-in-stock]",
    );
    const lowStockLabel = document.querySelector(
      "[data-variant-stock-display-low-stock]",
    );
    const outOfStockLabel = document.querySelector(
      "[data-variant-stock-display-out-of-stock]",
    );
    ...
    if (availablilityFlag === "pre-order") {
      ...
      dummyAtcButton
        ? (dummyAtcButton.innerHTML = dummyAtcButton.getAttribute(preOrderAttr))
        : null;
      ...
      dummyAtcButton
        ? (dummyAtcButton.innerHTML = dummyAtcButton.getAttribute(inStockAttr))
        : null;
      ...
      dummyAtcButton
        ? (dummyAtcButton.innerHTML = dummyAtcButton.getAttribute(oosAttr))
        : null;
      preOrderNotice ? preOrderNotice.classList.add("hide") : null;
      atcButton.removeAttribute("data-atc-pre-order");
    } else {
      ...
      dummyAtcButton
        ? (dummyAtcButton.innerHTML =
            dummyAtcButton.getAttribute(unavailableAttr))
        : null;
      preOrderNotice ? preOrderNotice.classList.add("hide") : null;
      atcButton.removeAttribute("data-atc-pre-order");
    }
