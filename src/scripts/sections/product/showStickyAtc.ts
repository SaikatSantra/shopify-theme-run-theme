const showStickyAtc = (): void => {
  const toggleElement = document.querySelector('.product-sticky-atc') as HTMLElement
  if (!toggleElement) {
    return;
  }

  const toggleTrigger: HTMLDivElement = document.querySelector('[data-product-form] [data-product-form-submit]')
