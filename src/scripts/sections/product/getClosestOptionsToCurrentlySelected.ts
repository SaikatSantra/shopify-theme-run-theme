...
const getClosestOptionsToCurrentlySelected = (
  currentOptions: OptionList,
  matchingOptions: OptionsMap[],
): OptionsMap | undefined => {
  const scores = matchingOptions.map((variant) =>
    variant.options.reduce(
      (acca, cv, index) => (currentOptions[index] === cv ? acca + 1 : acca),
      0,
    ),
  );
