  ...
  accountMenu.addEventListener('change', function (e) {
    ...
    if (dataset.changePassword) {
      axios.get(`${getRoute()}account/logout`)
        .then(() => window.location.href = `${getRoute()}account/login/#recover`)
...
export const initActiveMenuItem = (): void => {
  const accountMenus = document.querySelectorAll('[data-account-menu]') as NodeListOf<HTMLElement>;
  const mobileAccountMenus = document.querySelectorAll('[data-mobile-account-menu]') as NodeListOf<HTMLElement>;
  const mobileAccountTitles = document.querySelectorAll('[data-mobile-account-title]') as NodeListOf<HTMLElement>;
  ...
  if (queryString) {
    accountMenus.forEach(menu => {
      ...
      menuItems.forEach(item => {
        ...
        if (linkHref.includes(queryString)) {
          ...
          menuLink.hasAttribute('data-mobile-account-link') ? item.classList.remove('hide') : null;
        }
      })
    });

    mobileAccountTitles.forEach(title => {
      const accountTitles = title.querySelectorAll('span') as NodeListOf<HTMLElement>;
    ...
    mobileAccountTitles.forEach(title => {
      ...
      accountTitles.forEach(item => {
        const menuLink = item.querySelector('a') as HTMLElement;
        const linkHref = menuLink.getAttribute('href');
        if (linkHref === pathName || (linkHref.includes('orders') && window.location.href.includes('/orders'))) {
          item.classList.remove('hide');
        }
      });
    })
