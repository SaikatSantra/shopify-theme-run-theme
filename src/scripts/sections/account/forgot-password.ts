...
export const toggleForgotPassword = (): void => {
  ...
  toggles.forEach((toggle) => {
    toggle.addEventListener("click", (e) => {
      ...
      e.preventDefault();
      document
        .querySelector("[data-recover-form]")
        .classList.toggle(classToToggle);
      document
        .querySelectorAll("[data-login-form]")
        .forEach((element) => element.classList.toggle(classToToggle));
    });
  });
};
  ...
  toggles.forEach((toggle) => {
    toggle.addEventListener("click", () => {
      axios
        ...
        .then(
          () =>
            (window.location.href = `${getRoute()}account/login/#${recoverHash}`),
        )
