...
import { Navigation, Pagination, FreeMode, Thumbs, Autoplay, Zoom } from 'swiper/modules';
...
export const swipify = () => {
  ...
  swipers.forEach(swiper => {
    ...
    let config = swiper.dataset.swiperConfig ? safeJSONParse(swiper.dataset.swiperConfig, false) : {};
    const mods = [];
  ...
  const initSwiperOnMobile = () => {
    ...
    if (window.innerWidth < MOBILE_BREAKPOINT) {
      ...
      if (window.blubolt.swiperMobileObjects) {
        ...
        keys.forEach(key => window.blubolt.swiperMobileObjects[key].destroy());
        window.blubolt.swiperMobileObjects = [];
      }
    }
  };
