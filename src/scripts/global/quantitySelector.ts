...
const initQuantitySelector = (): void => {
  ...
  selectors.forEach((selector) => {
    const qtyInput: HTMLInputElement = selector.querySelector(
      "[data-quantity-input]",
    );
    const qtyDec: HTMLButtonElement = selector.querySelector(
      "[data-quantity-dec]",
    );
    const qtyInc: HTMLButtonElement = selector.querySelector(
      "[data-quantity-inc]",
    );
