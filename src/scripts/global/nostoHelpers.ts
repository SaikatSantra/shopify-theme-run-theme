...
const getSwatchImage = (productMetafields) : any => {
  const swatchContent = []
  const maxSwatches = 3;
  for (const [key, val] of Object.entries<variantMetafields>(productMetafields.variants)) {
    val.metafields.variant_image ? swatchContent.push(`<div class="product-card-swatches__image"><img width="80" height="80" src=${val.metafields.variant_image} alt=${key} /></div>`) : ''
  }
  ...
  if (swatchContent.length > maxSwatches) {
    ...
    swatchContent.push(`<div class="product-card-swatches__count">+${removeSwatches}</div>`)
  }
...
const getSwatchColour = (productMetafields) : any => {
  const swatchContent = []
  for (const [key, val] of Object.entries<variantMetafields>(productMetafields.variants)) {
    val.metafields.swatch_colour ? swatchContent.push(`<div class="product-card-swatches__swatch" style="--bg: ${val.metafields.swatch_colour};" title="${key}"></div>`) : ''
  }
  ...
  nostoElems.forEach((el) => {
    ...
    anchors.forEach(anchor => {
      ...
      if(!a.includes('/en')) {
        ...
        const position = anchor.href.indexOf('.com') ? anchor.href.indexOf('.com') + 4 : anchor.href.indexOf('.co.uk') + 4;
...
export const nostoSwatches = ():void => {
  ...
  const nostoSwatches = document.querySelectorAll('[data-nosto-swatches]') as NodeListOf<HTMLDivElement>;
  
  nostoSwatches.forEach(async (el) => {
    const isImages = el.classList.contains('product-card-swatches--images') ? true : false;
    const productHandle = el.dataset.nostoSwatches;
    const productData = await getTemplateJSON('products', productHandle, 'json-data')
