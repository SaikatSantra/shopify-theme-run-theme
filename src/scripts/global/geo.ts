...
export const initGeo = async (): Promise<void> => {
  try {
    ...
    const countryRedirectData = document.querySelector(
      "[data-geo-locales-list]",
    ) as HTMLElement;
    ...
    if (countryRedirectData) {
      ...
      desiredLocale = list.filter((l) =>
        l.localesNotToShow.includes(data.detected_values.country.handle),
      )[0];
    }

    const currencySelectorDT = document.querySelector(
      "[data-currency-selector-dt]",
    ) as HTMLElement;
    ...
    const dismisses = el.querySelectorAll(
      "[data-geo-dismiss]",
    ...
    let newUrl;
    const regionOption = el.querySelector(
      "[data-geo-switcher-region]",
    ) as HTMLSelectElement;
    const regionOptions = regionOption?.querySelectorAll(
      "option",
    ) as NodeListOf<HTMLOptionElement>;
    const languageOption = el.querySelector(
      "[data-geo-switcher-language]",
    ) as HTMLSelectElement;
    const languageOptions = languageOption?.querySelectorAll(
      "option",
    ) as NodeListOf<HTMLOptionElement>;
    ...
    regionOption?.addEventListener("change", () => {
      const regionTitle =
        regionOption.options[regionOption.selectedIndex].dataset.region;
      ...
      languageOptions?.forEach((option, i) => {
        if (option.dataset.region === selectedRegion) {
          ...
          if (
            i === 0 ||
            option.previousElementSibling.classList.contains("hide")
          ) {
            option.setAttribute("selected", "selected");
            newUrl = option.value;
          }
    ...
    languageOptions?.forEach((option, i) => {
      if (
        (!option.classList.contains("hide") && i === 0) ||
    ...
    countryEls.forEach((country) => {
      if (!window.location.href.includes(country.dataset.geoSwitcherDomain)) {
        ...
        document
          .querySelectorAll("[data-currency-selector-currency]")
          .forEach((el) => {
            el.innerHTML = country.dataset.geoSwitcherCurrency;
          });
        document
          .querySelectorAll("[data-currency-selector-language]")
          .forEach((el) => {
            el.innerHTML =
              languageOption.options[languageOption.selectedIndex].dataset.code;
          });
      }
    });
    ...
    const externalOpeners = document.querySelectorAll(
      "[data-geo-open]",
    ) as NodeListOf<HTMLElement>;
    ...
    //show on load??
    const show =
      window.localStorage.getItem("geoDismissed") !== handle &&
      !desiredLocale?.subFolder.includes(route);
