...
export const getSwatches = (): IGlobalSwatchSettings => {
  ...
  const swatchData = safeJSONParse(
    swatchDataEl.textContent,
    [],
  ) as ISwatchDatum[];
...
export const popuplateSwatches = (
  swatches: IGlobalSwatchSettings,
  container: HTMLElement | Document = document,
): void => {
  if (
    !swatches ||
    typeof swatches !== "object" ||
    Object.keys(swatches).length < 1
  ) {
