...
const initialiseZoom = (wrapperSelector, level) => {
  ...
  document.body.addEventListener("click", (evt) => {
    ...
    if (clickedOff && zoomImageState.zoomFlag) {
      zoomImageState = setZoom(
        zoomImageState.containerCentre,
        1,
        zoomImageState,
      );
    }
  ...
  zoomImageState.el.addEventListener("gestureend", (e) => {
    ...
    zoomImageState =
      e.scale > 1
        ? setUpPans(zoomImageState.containerCentre, zoomImageState)
        : zoomImageState;
  ...
  ["touchstart", "mousedown"].forEach((evt) => {
    container.addEventListener(evt, (e) => {
      ...
      zoomImageState = startPan(
        evtCoords(e),
        e.target === zoomImageState.el,
        zoomImageState,
      );
    });
  });
  ...
  zoomToggle.addEventListener("click", () => {
    zoomImageState = setZoom(
      zoomImageState.containerCentre,
      zoomImageState.zoomFlag ? 1 : zoomImageState.maxZoom,
      zoomImageState,
    );
    zoomImageState = updateUrl(zoomImageState);
  });
