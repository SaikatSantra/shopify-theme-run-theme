...
const initAccordion = () => {
  ...
  accordions.forEach(accordion => {
    const config = accordion.dataset.accordionConfig ? safeJSONParse(accordion.dataset.accordionConfig, false) : {};
    ...
    function openAccordion(e) {
      if (e.type === 'click' || e.key === 'Enter') {
        config.closeAllOnOpen ? toggles.forEach(toggle => onCloseAccordion(toggle)) : null;

        const content = e.target.nextElementSibling;
        const contentHeight = content.scrollHeight;
        e.target.classList.add('accordion__title--open');
        content.style.setProperty('--max-height', `${contentHeight + CONTENT_PADDING_BOTTOM}px`);
        content.style.setProperty('--padding', `0 0 ${CONTENT_PADDING_BOTTOM}px`);
    ...
    function closeAccordion(e) {
      e.type === 'click' || e.key === 'Enter' ? onCloseAccordion(e.target) : null;
    }
